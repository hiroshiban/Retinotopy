...
fixL=bgcircle;
fixL(bglength/2-outlength/2-size(fcross_vbar,1)/2+1:bglength/2-outlength/2+size(fcross_vbar,1)/2,bglength/2-size(fcross_vbar,2)/2+1:bglength/2+size(fcross_vbar,2)/2,1:3)=fcross_vbar; % top
fixL(bglength/2-size(fcross_hbar,1)/2+1:bglength/2+size(fcross_hbar,1)/2,bglength/2-outlength/2-size(fcross_hbar,2)/2+1:bglength/2-outlength/2+size(fcross_hbar,2)/2,1:3)=fcross_hbar; % left
fixL=uint8(fixL);

% add bars on the bottom & right positions
fixR=bgcircle;
fixR(bglength/2+outlength/2-size(fcross_vbar,1)/2+1:bglength/2+outlength/2+size(fcross_vbar,1)/2,bglength/2-size(fcross_vbar,2)/2+1:bglength/2+size(fcross_vbar,2)/2,1:3)=fcross_vbar; % bottom
fixR(bglength/2-size(fcross_hbar,1)/2+1:bglength/2+size(fcross_hbar,1)/2,bglength/2+outlength/2-size(fcross_hbar,2)/2+1:bglength/2+outlength/2+size(fcross_hbar,2)/2,1:3)=fcross_hbar; % right
fixR=uint8(fixR);
...
if show_flag
  ...
  subplot(1,2,1);
  imid1=imshow(fixL(:,:,1:3));
  set(imid1,'AlphaData',double(fixL(:,:,4)./255));
  hold on;

  subplot(1,2,2);
  imid2=imshow(fixR(:,:,1:3));
  set(imid2,'AlphaData',double(fixR(:,:,4)./255));
end

if save_flag
  save FixationImg.mat fixL fixR;
end
