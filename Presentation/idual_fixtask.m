...
% Last Update: "2024-02-08 18:10:13 ban"
%
%
%
% [input variables]
...
logfname=fullfile(resultDir,[num2str(subjID),'_idual_fixtask_',exp_mode,'_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
for nn=1:1:length(checkerboard)
  checkerboard{nn}=255.*(~checkerboard{nn});
  checkerboard{nn}=repmat(checkerboard{nn},[1,1,4]);
...
if strfind(upper(subjID),'DEBUG')
  ...
  save_dir=fullfile(resultDir,'images_idual_fixtask');
  ...
  for nn=1:1:length(checkerboard), checkertexture{nn}=Screen('MakeTexture',winPtr,checkerboard{nn}); end

  % check the number of flickers
  if mod((sparam.pol_cycle_duration-sparam.pol_rest_duration)/(360/sparam.pol_rotangle)/sparam.flip_duration,1)~=0
    warning('(sparam.pol_cycle_duration-sparam.pol_rest_duration)/(360/sparam.pol_rotangle)/sparam.flip_duration is not an integer. check the sparam parameters.');
  end

  % processing
  obj_counter=0;
  for nn=1:1:length(checkerboard)
    for cc=1:1:round((sparam.pol_cycle_duration-sparam.pol_rest_duration)/(360/sparam.pol_rotangle)/sparam.flip_duration);
      ...
      for pp=1:1:2 % by default, 2 image sets in one background noise flicker
        ...
        for nnnn=1:1:nScr
          ...
          imwrite(Screen('GetImage',winPtr,winRect),fullfile(save_dir,sprintf('retinotopy_%s_pos_%02d_type_%02d_%02d_stereo_%02d.png',sparam.mode,nn,cc,pp,nnnn)),'png');
        end
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_idual_fixtask_',sparam.mode,'_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_idual_fixtask_',sparam.mode,'_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
