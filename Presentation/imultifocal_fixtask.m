...
% Object-image-defined multi-focal retinotopy stimulus with fixation luminance change-detection tasks.
% function imultifocal_fixtask(subjID,exp_mode,acq,:displayfile,:stimulusfile,:gamma_table,:overwrite_flg,:force_proceed_flag)
% (: is optional)
%
...
% Last Update: "2023-10-26 16:12:46 ban"
%
%
%
% [input variables]
...
logfname=fullfile(resultDir,[num2str(subjID),'_imultifocal_fixtask_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
[dummy1,dummy2,mfcheckerboard]=mf_GenerateCheckerBoard1D(rmin,rmax,sparam.width,sparam.ndivsP,sparam.ndivsE,sparam.startangle,sparam.pix_per_deg,1,1,0);
...
for cc=1:1:sparam.numTrials
  ...
  checkerboard{cc}=255.*(~cmask);
end
clear cmask mask_idx mfcheckerboard;

%% organize the checkerboard into masks
for cc=1:1:sparam.numTrials
  checkerboard{cc}=repmat(checkerboard{cc},[1,1,4]);
  checkerboard{cc}(:,:,1:3)=repmat(reshape(sparam.bgcolor,[1,1,3]),[size(checkerboard{cc},1),size(checkerboard{cc},2)]);
end
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_imultifocal_fixtask_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_imultifocal_fixtask_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
return;
% end % function imultifocal_fixtask
