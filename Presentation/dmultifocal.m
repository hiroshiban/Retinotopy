function dmultifocal(subjID,exp_mode,acq,displayfile,stimulusfile,gamma_table,overwrite_flg,force_proceed_flag)

% Random-Dot-Stereogram(RDS)-defined multi-focal retinotopy checkerboard stimulus with checker-patch depth change-detection tasks.
% function dmultifocal(subjID,exp_mode,acq,:displayfile,:stimulusfile,:gamma_table,:overwrite_flg,:force_proceed_flag)
% (: is optional)
%
...
% ./subjects/(subjID)/results/(date)/(subjID)_dmultifocal_results_run_(run_num).mat.
...
% Last Update: "2021-06-07 17:19:32 ban"
%
%
%
% [input variables]
...
%    as ./subjects/(subjID)/results/(date)/(subjID)_dmultifocal_results_run_(run_num).mat
%
%
% [example]
% >> dmultifocal('HB','ccw',1,'ret_display.m','ret_checker_stimulus_exp1.m')
%
% [About displayfile]
...
logfname=fullfile(resultDir,[num2str(subjID),'_dmultifocal_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_dmultifocal_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_dmultifocal_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
