function dmeridian_fixtask(subjID,exp_mode,acq,displayfile,stimulusfile,gamma_table,overwrite_flg,force_proceed_flag)

% Random-Dot-Stereogram(RDS)-defined dual checkerboard stimulus along horizontal/vertical meridians with fixation luminance change-detection tasks.
% function dmeridian_fixtask(subjID,exp_mode,acq,:displayfile,:stimulusfile,:gamma_table,:overwrite_flg,:force_proceed_flag)
% (: is optional)
%
...
%
%
% Created    : "2019-05-20 17:53:42 ban"
% Last Update: "2019-05-24 13:23:08 ban"
%
%
%
% [input variables]
...
%    as ./subjects/(subjID)/results/(date)/(subjID)_dmeridian_fixtask_results_run_(run_num).mat
%
%
% [example]
% >> dmeridian_fixtask('HB','meridian',1,'ret_display.m','ret_checker_stimulus_exp1.m')
%
% [About displayfile]
...
logfname=fullfile(resultDir,[num2str(subjID),'_dmeridian_fixtask_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
[dummy,checkerboard]=pol_GenerateCheckerBoard1D(rmin,rmax,sparam.width,sparam.startangles,sparam.pix_per_deg,...
...
if sparam.RDSbackground
  ...
  for nn=1:1:numel(sparam.startangles), checkerboard{nn}(~logical(mask{1}))=NaN; end
else
  for nn=1:1:numel(sparam.startangles), checkerboard{nn}(~logical(checkerboard{nn}))=NaN; end
end
...
if strfind(upper(subjID),'DEBUG')
  ...
  save_dir=fullfile(resultDir,'images_dmeridian_fixtask');
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_dmeridian_fixtask_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_dmeridian_fixtask_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
return;
% end % function dmeridian_fixtask
