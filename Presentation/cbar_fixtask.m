...
%
% [note]
% Behavioral task of cbar_fixtask is to detect changes of luminance of the central
% fixation period, whereas tasks in cbar is to detect changes of luminance of one
...
logfname=fullfile(resultDir,[num2str(subjID),'_cbar_ftask_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
totalframes=2*max(nframe_fixation,1)+numel(sparam.rotangles)*(nframe_cycle+nframe_rest)*sparam.numRepeats;
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_cbar_ftask_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_cbar_ftask_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
