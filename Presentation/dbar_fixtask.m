function dbar_fixtask(subjID,exp_mode,acq,displayfile,stimulusfile,gamma_table,overwrite_flg,force_proceed_flag)

% Random-Dot-Stereogram(RDS)-defined checkerboard bar stimulus (pRF) with checker-patch luminance change-detection tasks.
% function dbar_fixtask(subjID,exp_mode,acq,:displayfile,:stimulusfile,:gamma_table,:overwrite_flg,:force_proceed_flag)
% (: is optional)
%
...
% Last Update: "2021-06-07 17:21:48 ban"
%
%
%
% [input variables]
...
%    as ./subjects/(subjID)/results/(date)/(subjID)_dbar_fixtask_results_run_(run_num).mat
%
%
% [example]
% >> dbar_fixtask('HB','bar',1,'bar_display.m','bar_stimulus.m')
%
% [About displayfile]
...
logfname=fullfile(resultDir,[num2str(subjID),'_dbar_fixtask_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
[dummy,checkerboard]=bar_GenerateCheckerBar1D(fieldSize,width,sparam.rotangles,sparam.steps,...
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_dbar_fixtask_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_dbar_fixtask_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
