...
% Last Update: "2019-04-04 17:44:38 ban"
%
%
%
% [input variables]
...
%    as ./subjects/(subjID)/results/(date)/(subjID)_chrf_fixtask_results_run_(run_num).mat
%
%
% [example]
...
logfname=fullfile(resultDir,[num2str(subjID),'_chrf_fixtask_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
if strfind(upper(subjID),'DEBUG')
  ...
  Screen('CloseAll');
  save_dir=fullfile(resultDir,'images_chrf_fixtask');
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_chrf_fixtask_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_chrf_fixtask_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
