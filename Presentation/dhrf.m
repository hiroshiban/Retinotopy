...
% Random-Dot-Stereogram(RDS)-defined checkerboard stimulus with checker-patch depth change detection tasks for measuring and estimating a HRF function.
% function dhrf(subjID,exp_mode,acq,:displayfile,:stimulusfile,:gamma_table,:overwrite_flg,:force_proceed_flag)
% (: is optional)
%
...
% Last Update: "2023-11-06 16:50:56 ban"
%
%
%
% [input variables]
...
logfname=fullfile(resultDir,[num2str(subjID),'_dhrf_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
task_pos=randi(numel(unique(checkerboardID))-1,[ceil(sparam.numRepeats*2*(nframe_stim+nframe_rest)/nframe_task),1]);
...
for cc=1:1:sparam.numRepeats
  ...
  for ff=1:1:nframe_stim+nframe_rest
    ...
    if ff<=nframe_stim && ~mod(ff,nframe_flicker)
      ...
      if ~isempty(tidx)
        if cval==1
          [XY,colors]=GetDotPositionsRDS(checkerboard,[0,depth1,depth2,sparam.RDStaskmagnitude*(-1)*abs(sparam.RDSdepth(1))],sparam.RDSDense,...
                                         sparam.RDScolors(1:2),sparam.ipd,sparam.vdist,sparam.pix_per_cm);
        else % if cval==2
          [XY,colors]=GetDotPositionsRDS(checkerboard,[0,depth1,depth2,sparam.RDStaskmagnitude*(-1)*abs(sparam.RDSdepth(2))],sparam.RDSDense,...
                                         sparam.RDScolors(1:2),sparam.ipd,sparam.vdist,sparam.pix_per_cm);
        end
        checkerboard(tidx)=cval; % put the checkerboard ID back to the default
      else
          [XY,colors]=GetDotPositionsRDS(checkerboard,[0,depth1,depth2],sparam.RDSDense,...
                                         sparam.RDScolors(1:2),sparam.ipd,sparam.vdist,sparam.pix_per_cm);
      end
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_dhrf_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_dhrf_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
