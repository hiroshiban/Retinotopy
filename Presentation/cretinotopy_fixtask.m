...
% Last Update: "2018-11-22 16:27:41 ban"
%
%
%
% [input variables]
...
logfname=fullfile(resultDir,[num2str(subjID),'_cretinotopy_ftask_',exp_mode,'_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
for cc=1:1:sparam.numRepeats
  ...
  for ff=1:1:nframe_cycle
    ...
    for nn=1:1:nScr
      ...
      DrawTextureWithCLUT(winPtr,checkertexture{stim_pos_id},CLUT{stim_pos_id}{color_id,compensate_id},[],CenterRect(stimRect,winRect));
    ...
    Screen('DrawingFinished',winPtr);
    if strcmpi(sparam.mode,'cw') || strcmpi(sparam.mode,'cont')
      Screen('Flip',winPtr,vbl+sparam.initial_fixation_time+(cc-1)*sparam.cycle_duration+sparam.rest_duration+((ff-1)*sparam.waitframes-0.5)*dparam.ifi,[],[],1);
    else
      Screen('Flip',winPtr,vbl+sparam.initial_fixation_time+(cc-1)*sparam.cycle_duration+((ff-1)*sparam.waitframes-0.5)*dparam.ifi,[],[],1);
    end
    cur_frames=cur_frames+1;
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_cretinotopy_ftask',sparam.mode,'_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_cretinotopy_ftask',sparam.mode,'_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
return;
% end % function cretinotopy_fixtask
