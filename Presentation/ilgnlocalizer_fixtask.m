function ilgnlocalizer_fixtask(subjID,exp_mode,acq,displayfile,stimulusfile,gamma_table,overwrite_flg,force_proceed_flag)

% Object-image-defined wedge stimulus with fixation luminance change-detection tasks, for localizing LGN.
% function ilgnlocalizer_fixtask(subjID,exp_mode,acq,:displayfile,:stimulusfile,:gamma_table,:overwrite_flg,:force_proceed_flag)
% (: is optional)
%
% - This function generates and presents wedge stimuli in the left and right
%   visual hemifield alternatively, for identifying LGN subject-by-subject.
...
% Last Update: "2019-04-04 11:53:25 ban"
%
%
%
% [input variables]
...
%    as ./subjects/(subjID)/results/(date)/(subjID)_ilgn_localizer_fixtask_results_run_(run_num).mat
%
%
% [example]
% >> ilgnlocalizer_fixtask('HB','localizer',1,'ret_display.m','ret_checker_stimulus_exp1.m')
%
% [About displayfile]
...
%
% % ************************************************************
% % This is the stimulus parameter file for the ilgnlocalizer retinotopy stimulus
...
logfname=fullfile(resultDir,[num2str(subjID),'_ilgn_localizer_fixtask_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
if strfind(upper(subjID),'DEBUG')
  ...
  save_dir=fullfile(resultDir,'images_ilgn_localizer_fixtask');
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_ilgn_localizer_fixtask_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_ilgn_localizer_fixtask_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
return;
% end % function ilgnlocalizer_fixtask
