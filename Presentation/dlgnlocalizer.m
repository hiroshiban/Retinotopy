function dlgnlocalizer(subjID,exp_mode,acq,displayfile,stimulusfile,gamma_table,overwrite_flg,force_proceed_flag)

% Random-Dot-Stereogram(RDS)-defined checkerboard stimulus with checker-patch depth change-detection tasks, for localizing LGN.
% function dlgnlocalizer(subjID,exp_mode,acq,:displayfile,:stimulusfile,:gamma_table,:overwrite_flg,:force_proceed_flag)
% (: is optional)
%
...
%
%
% Created    : "2019-05-23 10:38:50 ban"
% Last Update: "2019-05-24 10:52:24 ban"
%
%
%
% [input variables]
...
%    as ./subjects/(subjID)/results/(date)/(subjID)_dlgn_localizer_results_run_(run_num).mat
%
%
% [example]
% >> dlgnlocalizer('HB','localizer',1,'ret_display.m','ret_checker_stimulus_exp1.m')
%
% [About displayfile]
...
logfname=fullfile(resultDir,[num2str(subjID),'_dlgn_localizer_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
task_pos=cell(2,1); % 2 = left and right visual hemifields
for nn=1:1:2
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_dlgn_localizer_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_dlgn_localizer_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
