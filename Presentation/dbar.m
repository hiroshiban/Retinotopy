...
% Random-Dot-Stereogram(RDS)-defined checkerboard bar stimulus (pRF) with checker-patch depth change-detection tasks.
% function dbar(subjID,exp_mode,acq,:displayfile,:stimulusfile,:gamma_table,:overwrite_flg,:force_proceed_flag)
% (: is optional)
%
...
% Last Update: "2023-11-06 16:47:35 ban"
%
%
%
% [input variables]
...
logfname=fullfile(resultDir,[num2str(subjID),'_dbar_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
for aa=1:1:numel(sparam.rotangles)
  for nn=1:1:sparam.steps
    ...
    patchids{aa,nn}=tmp_checks(2:end); % omit background id
  end
end
clear tmp_checks;
...
if sparam.RDSbackground
  ...
  for aa=1:1:numel(sparam.rotangles)
    for nn=1:1:sparam.steps
      checkerboard{aa,nn}(~logical(mask{1}))=NaN;
    end
  end
else
  for aa=1:1:numel(sparam.rotangles)
    for nn=1:1:sparam.steps
      checkerboard{aa,nn}(~logical(checkerboard{aa,nn}))=NaN;
    end
  end
end

% generating the first RDS
[XY,colors]=GetDotPositionsRDS(checkerboard{1,1},[0,sparam.RDSdepth(1),sparam.RDSdepth(2)],sparam.RDSDense,...
...
if strfind(upper(subjID),'DEBUG')
  ...
  save_dir=fullfile(resultDir,'images_dbar');
  ...
  for aa=1:1:numel(sparam.rotangles)
    for nn=1:1:sparam.steps
      for rr=1:1:round(nframe_stim/(sparam.steps*nframe_flicker*sparam.RDSdepth(3)))
        for mm=1:1:sparam.RDSdepth(3) % depth steps
          ...
          [XY,colors]=GetDotPositionsRDS(checkerboard{aa,nn},[0,depth1,depth2],sparam.RDSDense,...
                                         sparam.RDScolors(1:2),sparam.ipd,sparam.vdist,sparam.pix_per_cm);
          ...
          for pp=1:1:2 % left and right images
            ...
            imwrite(Screen('GetImage',winPtr,winRect),fullfile(save_dir,sprintf('bar_angle_%02d_pos_%02d_%02d_depth_%02d_%02d.png',aa,nn,rr,mm,pp)),'png');
          end
        end
      end
    end
...
for cc=1:1:sparam.numRepeats
  ...
  for aa=1:1:numel(sparam.rotangles)
    ...
    for ff=1:1:nframe_stim+nframe_rest
      ...
      if ff<=nframe_stim && (~mod(ff,nframe_flicker) || ~mod(ff,nframe_movement))
        ...
        if do_task(task_id) && ...
          ...
          cval=checkerboard{aa,stim_pos_id}(tidx(ceil(numel(tidx)/2))); % ceil(numel(tidx)/2) is required as sometimes tidx(1) is located at the edge of the checkerboard which gives NaN.
          checkerboard{aa,stim_pos_id}(tidx)=3; % as IDs of the original checkerboard are 0|1|2, 3 is assigned to the task patch.
        else
          tidx=[];
        end
        ...
        if ~isempty(tidx)
          if cval==1
            [XY,colors]=GetDotPositionsRDS(checkerboard{aa,stim_pos_id},[0,depth1,depth2,sparam.RDStaskmagnitude*(-1)*abs(sparam.RDSdepth(1))],sparam.RDSDense,...
                                           sparam.RDScolors(1:2),sparam.ipd,sparam.vdist,sparam.pix_per_cm);
          else % if cval==2
            [XY,colors]=GetDotPositionsRDS(checkerboard{aa,stim_pos_id},[0,depth1,depth2,sparam.RDStaskmagnitude*(-1)*abs(sparam.RDSdepth(2))],sparam.RDSDense,...
                                           sparam.RDScolors(1:2),sparam.ipd,sparam.vdist,sparam.pix_per_cm);
          end
          checkerboard{aa,stim_pos_id}(tidx)=cval; % put the checkerboard ID back to the default
        else
            [XY,colors]=GetDotPositionsRDS(checkerboard{aa,stim_pos_id},[0,depth1,depth2],sparam.RDSDense,...
                                           sparam.RDScolors(1:2),sparam.ipd,sparam.vdist,sparam.pix_per_cm);
        end
      end % if ff<=nframe_stim && (~mod(ff,nframe_flicker) || ~mod(ff,nframe_movement))
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_dbar_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_dbar_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
