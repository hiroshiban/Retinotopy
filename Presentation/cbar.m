...
% Last Update: "2018-11-22 16:27:53 ban"
%
%
%
% [input variables]
...
for aa=1:1:numel(sparam.rotangles)
  for nn=1:1:sparam.steps
    CLUT{aa,nn}=cell(1+true_npatches{aa,nn},sparam.ncolors,2); % 1+npatches is base + task CLUTs, 2 is for compensating patterns
  end
end
  ...
  for nn=1:1:sparam.steps
    for cc=1:1:sparam.ncolors
      for pp=1:1:2 % compensating checkers
        ...
        CLUT{aa,nn}{1,cc,pp}=zeros(256,4);
        CLUT{aa,nn}{1,cc,pp}(:,4)=1; % default alpha is 1 (no transparent)
        CLUT{aa,nn}{1,cc,pp}(1,:)=[sparam.colors(1,:) 0]; % background LUT, default alpha is 0 (invisible);
        ...
        if pp==1
          for vv=patchids{aa,nn}
            if mod(ceil(vv/sparam.ndivsL),2)
              if mod(vv,2)
                CLUT{aa,nn}{1,cc,pp}(vv+1,1:3)=sparam.colors(2*cc,:);
              else
                CLUT{aa,nn}{1,cc,pp}(vv+1,1:3)=sparam.colors(2*cc+1,:);
              end
            else
...
for aa=1:1:numel(sparam.rotangles)
  for nn1=1:1:sparam.steps
    for nn2=2:1:true_npatches{aa,nn1}+1 % = number of task positions
      for cc=1:1:sparam.ncolors
        for pp=1:1:2 % compensating checkers
          CLUT{aa,nn1}{nn2,cc,pp}=CLUT{aa,nn1}{1,cc,pp};
          % in the code below, +1 is required as CLUT{x,x}{x,x,x}(1,:) is background LUT.
          CLUT{aa,nn1}{nn2,cc,pp}(patchids{aa,nn1}(nn2-1)+1,1:3)=round(0.4*CLUT{aa,nn1}{nn2,cc,pp}(patchids{aa,nn1}(nn2-1)+1,1:3));
        end
      end
    end
  end
...
if strfind(upper(subjID),'DEBUG')
  ...
  for aa=1:1:numel(sparam.rotangles)
    for nn=1:1:sparam.steps
      ...
      colormap(CLUT{aa,nn}{1,1,1}(1:true_npatches{aa,nn}+1,1:3)./255);
    ...
    for ff=1:1:nframe_cycle
      ...
      for nn=1:1:nScr
        ...
        if do_task(task_id) && ...
          ...
          DrawTextureWithCLUT(winPtr,checkertexture{aa,stim_pos_id},CLUT{aa,stim_pos_id}{task_pos{aa,stim_pos_id}(task_id)+1,color_id,compensate_id},[],CenterRect(stimRect,winRect));
        else
          DrawTextureWithCLUT(winPtr,checkertexture{aa,stim_pos_id},CLUT{aa,stim_pos_id}{1,color_id,compensate_id},[],CenterRect(stimRect,winRect));
        end
