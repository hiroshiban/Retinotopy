...
nframe_task=round(nframe_trial/2);
...
task_flg=randi(2,[round(sparam.numTrials*nframe_trial/nframe_task),1]);

% flag whether presenting disparity task
do_task=zeros(round(sparam.numTrials*nframe_trial/nframe_task),1);
do_task(1)=0; % no task for the first presentation
for ii=2:1:round(sparam.numTrials*nframe_trial/nframe_task)
  if do_task(ii-1)==1
    do_task(ii)=0;
  else
...
for cc=1:1:sparam.numTrials
  task_pos{cc}=[];
  for pp=1:1:round(sparam.numTrials*nframe_trial/nframe_task)
    tmp_id=shuffle(patchids{cc});
    task_pos{cc}=[task_pos{cc},tmp_id(1)];
  end
end
...
for cc=1:1:sparam.numTrials
  ...
  for ff=1:1:nframe_trial
    ...
    if do_task(task_id) && ...
      ( ( task_flg(task_id)==1 && mod(ff,nframe_trial)<=nframe_trial/2 ) || ...
        ( task_flg(task_id)==2 && mod(ff,nframe_trial)>nframe_trial/2 ) )
