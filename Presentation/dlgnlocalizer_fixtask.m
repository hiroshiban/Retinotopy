function dlgnlocalizer_fixtask(subjID,exp_mode,acq,displayfile,stimulusfile,gamma_table,overwrite_flg,force_proceed_flag)

% Random-Dot-Stereogram(RDS)-defined checkerboard stimulus with fixation-point luminance change-detection tasks, for localizing LGN.
% function dlgnlocalizer_fixtask(subjID,exp_mode,acq,:displayfile,:stimulusfile,:gamma_table,:overwrite_flg,:force_proceed_flag)
% (: is optional)
%
...
% Last Update: "2019-05-23 15:53:55 ban"
%
%
%
% [input variables]
...
%    as ./subjects/(subjID)/results/(date)/(subjID)_dlgn_localizer_fixtask_results_run_(run_num).mat
%
%
% [example]
% >> dlgnlocalizer_fixtask('HB','localizer',1,'ret_display.m','ret_checker_stimulus_exp1.m')
%
% [About displayfile]
...
logfname=fullfile(resultDir,[num2str(subjID),'_dlgn_localizer_fixtask_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
if sparam.RDSbackground
  ...
  for nn=1:1:2, checkerboard{nn}(~logical(mask{1}))=NaN; end
else
...
if strfind(upper(subjID),'DEBUG')
  ...
  save_dir=fullfile(resultDir,'images_dlgnlocalizer_fixtask');
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_dlgn_localizer_fixtask_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_dlgn_localizer_fixtask_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
return;
% end % function dlgnlocalizer_fixtask
