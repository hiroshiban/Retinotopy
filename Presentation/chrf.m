...
% Last Update: "2019-02-01 16:08:21 ban"
%
%
%
% [input variables]
...
task_flg=randi(2,[round(sparam.numRepeats*2*(nframe_cycle+nframe_rest)/nframe_task),1]);

% flag whether presenting disparity task
do_task=zeros(round(sparam.numRepeats*2*(nframe_cycle+nframe_rest)/nframe_task),1);
do_task(1)=0; % no task for the first presentation
for ii=2:1:round(sparam.numRepeats*2*(nframe_cycle+nframe_rest)/nframe_task)
  if do_task(ii-1)==1
    do_task(ii)=0;
  else
...
task_pos=randi(numel(unique(checkerboardID))-1,[round(sparam.numRepeats*2*(nframe_cycle+nframe_rest)/nframe_task),1]);
...
for cc=1:1:sparam.numRepeats
  ...
  for ff=1:1:nframe_cycle
    ...
    if ~isempty(tidx), checkerboard(tidx)=checkerboard(tidx)-2; end
