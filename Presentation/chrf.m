function chrf(subjID,exp_mode,acq,displayfile,stimulusfile,gamma_table,overwrite_flg,force_proceed_flag)

% Color/luminance-defined checkerboard stimulus with checker-patch luminance change-detection tasks for measuring and estimating a HRF function.
% function chrf(subjID,exp_mode,acq,:displayfile,:stimulusfile,:gamma_table,:overwrite_flg,:force_proceed_flag)
% (: is optional)
%
...
% Last Update: "2019-02-22 15:21:37 ban"
%
%
%
% [input variables]
...
%    as ./subjects/(subjID)/results/(date)/(subjID)_chrf_results_run_(run_num).mat
%
%
% [example]
% >> chrf('HB','hrf',1,'ret_display.m','ret_checker_stimulus_exp1.m')
%
% [About displayfile]
...
logfname=fullfile(resultDir,[num2str(subjID),'_chrf_results_run_',num2str(acq,'%02d'),'.log']);
diary(logfname);
...
if strfind(upper(subjID),'DEBUG')
  ...
  Screen('CloseAll');
  save_dir=fullfile(resultDir,'images_chrf');
...
task_flg=randi(2,[ceil(sparam.numRepeats*2*(nframe_cycle+nframe_rest)/nframe_task),1]);

% flag whether presenting disparity task
do_task=zeros(ceil(sparam.numRepeats*2*(nframe_cycle+nframe_rest)/nframe_task),1);
do_task(1)=0; % no task for the first presentation
for ii=2:1:ceil(sparam.numRepeats*2*(nframe_cycle+nframe_rest)/nframe_task)
  if do_task(ii-1)==1
    do_task(ii)=0;
  else
...
task_pos=randi(numel(unique(checkerboardID))-1,[ceil(sparam.numRepeats*2*(nframe_cycle+nframe_rest)/nframe_task),1]);
...
for cc=1:1:sparam.numRepeats
  ...
  for ff=1:1:nframe_cycle+nframe_rest
    ...
    if ff<=nframe_cycle && ~isempty(tidx), checkerboard(tidx)=checkerboard(tidx)-2; end
...
% save data
savefname=fullfile(resultDir,[num2str(subjID),'_chrf_results_run_',num2str(acq,'%02d'),'.mat']);

% backup the old file(s)
if ~overwrite_flg
  BackUpObsoleteFiles(fullfile('subjects',num2str(subjID),'results',today),...
                      [num2str(subjID),'_chrf_results_run_',num2str(acq,'%02d'),'.mat'],'_old');
end
